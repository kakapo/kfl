...
require 'fdfs_storage_client1.php';
...
$result = storage_upload_by_filename1($tracker_server, $storage_server,
...
if ($file_buff != false)
{
	...
	$result = storage_upload_by_filebuff1($tracker_server, $storage_server,
...
$result = tracker_query_storage_fetch1($tracker_server, $storage_server, $file_id);
if ($result == 0)
{
	echo "storage server ${storage_server['ip_addr']}:${storage_server['port']}\n";
}
...
$result = storage_download_file_to_file1($tracker_server, $storage_server, 
...
$result = storage_download_file_to_buff1($tracker_server, $storage_server, 
                $file_id, $file_buff, $file_size);
if ($result == 0)
{
	echo "download file to buff success, file size: $file_size" . ", buff size:" . strlen($file_buff) . "\n";
	file_put_contents(str_replace('/', '_', $file_id), $file_buff);
}
else
{
...
$local_filename = str_replace('/', '-', $file_id);
...
{
	$result = storage_download_file_ex1($tracker_server, $storage_server, 
                $file_id, 'write_file_callback', $fp, $file_size);
	if ($result == 0)
	{
		echo "download file to file success, file size: $file_size\n";
	}
...
$result = storage_set_metadata1($tracker_server, $storage_server, $file_id, 
		$meta_list, STORAGE_SET_METADATA_FLAG_MERGE);
echo "set metadata result: $result\n";

sleep(1); //wait for file replication
$result = storage_get_metadata1($tracker_server, $storage_server, $file_id,  
		$meta_list);
if ($result == 0)
{
	var_dump($meta_list);
...
$result = storage_delete_file1($tracker_server, $storage_server, $file_id);
