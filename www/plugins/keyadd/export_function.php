...
function execute_curl($url, $referrer, $method, $post_data = "", $extra_type = "", $extra_data = "") {
	...
	if ($method == "get") {
		...
		if ($extra_type != "nocookie") {
			curl_setopt ( $c, CURLOPT_COOKIE, (($extra_type == "cookie") ? $extra_data : "") );
		}
...
function findinside2($start, $end, $string) {
	preg_match_all ( '/' . $start . '([^\.)]+)' . preg_quote ( $end, '/' ) . '/i', $string, $m );
	return $m;
}
...
function get_headerArrByString($string) {
	...
	$final = array ('location' => '', 'cookie' => $cookiearr, 'Server' => $Server, 'Date' => $Date, 'Content-Type' => $Content_Type, 'Transfer-Encoding' => $Transfer_Encoding, 'Connection' => $Connection, 'Cache-Control' => $Cache_Control );
	return $final;
}
