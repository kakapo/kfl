...
class OpenApp_51 {
	...
	public function check_user() {
		$this->fb_params = $this->decode_params ( $_POST, 5 * 60, $this->app_secret, '51_sig' );
		if (! $this->fb_params) {
			$this->fb_params = $this->decode_params ( $_GET, 5 * 60, $this->app_secret, '51_sig' );
		}
		if ($this->fb_params) {
		...
		} else if (! empty ( $_COOKIE ) && $cookies = $this->decode_params ( $_COOKIE, 9 * 3600, $this->app_secret, $this->app_key )) {
    ...
    */
	...
	public function get_add_url($next = null) {
		return self::get_OpenApp_51_url () . '/add.php?app_key=' . $this->app_key . ($next ? '&next=' . urlencode ( $next ) : '');
	}
	...
	public function set_user($user, $session_key, $time) {
		...
		foreach ( $cookies as $name => $val ) {
			...
			setcookie ( $this->app_key . '_' . $name, $val );
			$_COOKIE [$this->app_key . '_' . $name] = $val;
		}
		setcookie ( $this->app_key, $sig );
